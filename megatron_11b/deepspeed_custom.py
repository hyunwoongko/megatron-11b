"""
Deepspeed Inference Engine Custom
"""
...
from deepspeed.runtime.weight_quantizer import WeightQuantization
from deepspeed.module_inject.replace_module import replace_transformer_layer
...
class InferenceEngine(Module):
    ...
    def _validate_args(self, mpu):
        ...
        assert self.injection_dict is None or isinstance(
    ...
    def _load_checkpoint(self, load_dir, load_module_strict=True):
        ...
        mp_rank = 0 if self.mp_group is None else dist.get_rank(
            group=self.mp_group)
    ...
    def _convert_to_dtype(self):
        if self.dtype is torch.int8 and self.quantization_scales is None:
            ...
            model, self.quantization_scales = quantizer.model_quantize(
                self.module, self.injection_dict, self.quatize_bits,
                self.quantize_groups)
